...
class PoseEstimator(Module):
    ...
    def __init__(self, wheelParams: list[WheelParams], calibIMU:bool=False) -> None:
        ...
        self.sensorMSE = SensorData(self.numWheels, torch.ones(SensorData.size(self.numWheels)))
    ...
    def align(self, sensors: SensorData) -> bool:
        ...
        if self.alignCnt >= 100:
            ...
            self.sensorMSE.body = (((self.alignSum2.body / n) - (self.alignSum.body / n) ** 2) * torch.tensor((1., 1., 1., 10., 10., 10.))).detach()
            ...
            for s, s2, p, k in zip(self.alignSum.wheels, self.alignSum2.wheels, self.wheelParams, range(self.numWheels)):
                ...
                self.sensorMSE.wheels[k] = ((s2 / n) - (s / n) ** 2).detach()
            ...
            print(f"\nEstimated IMU MSE:")
            print(f"body: {self.sensorMSE.body}")
            for k, w in enumerate(self.sensorMSE.wheels):
                print(f"wheel #{k+1}: {w}")
    ...
    def forward(self, state: RobotState, sensors: SensorData, dt: Tensor) -> tuple[RobotState, RobotState]:
        ...
        # AHRS
        rpyInput = sensors.body[0:3]
        rpyInputCov = torch.diag(self.sensorMSE.body[0:3])
        rpyOutput = sensors.body[3:6]
        rpyOutputCov = torch.diag(self.sensorMSE.body[3:6])
        ...
        wheelInput = torch.cat([wd[0:3] for wd in sensors.wheels])
        wheelInputCov = torch.diag(torch.cat([we[0:3] for we in self.sensorMSE.wheels]))
        wheelOutput = torch.cat([wd[3:6] for wd in sensors.wheels])
        wheelOutputCov = torch.diag(torch.cat([we[3:6] for we in self.sensorMSE.wheels]))
        ...
        poseInput = torch.cat([wd[2:6] for wd in sensors.wheels])
        poseInputCov = torch.diag(torch.cat([we[2:6] for we in self.sensorMSE.wheels]))
        poseOutput = wheelInput
        poseOutputCov = wheelInputCov
    ...
    def rpy_predict(self, rpy: Tensor, u: Tensor) -> Tensor:
        """ Predicts new Euler angles from body gyro measurements """
        ...
        return rpy + gyro2euler @ u * self.dt
    ...
    def wheel_output(self, s: Tensor) -> Tensor:
        ...
        return torch.cat([
            (
                ...
                ).detach() # hub acceleration, sensitive only to wheel attitude
                + torch.stack((- params.ro * (ws ** 2), z, z)) # centrifugal acceleration from wheel own rotation
            )
            for ws, dcm, params in zip(wheelSpeeds, wheel2body, self.wheelParams)
        ])

    def pose_predict(self, s: Tensor, u: Tensor) -> Tensor:
        """ Predicts new state (linVelocity, xyz) from wheel (gz, ax, ay, az) """
        ...
        wheelsData = u.reshape(self.numWheels, 4)

        accels = [
            dcm @ (wd[1:4] + torch.tensor((params.ro*wd[0]**2, 0., 0.)))
                - torch.cross(angVelocity, torch.cross(angVelocity, params.xyz, dim=0), dim=0)
            for wd, dcm, params in zip(wheelsData, wheel2body, self.wheelParams)
        ]
    ...
    def pose_output(self, s: Tensor) -> Tensor:
        """ Returns the expected wheel gyro data from pose state """
        linVelocity = s[0:3]
        # xyz = s[3:6]
        angVelocity = self.state.angVelocity
        ...
        return torch.cat([
            (
                dcm.T @ (angVelocity + torch.stack((z, z, ss))).detach() # body rotation + steering
                + torch.stack((z, z, ws * sgn)) # wheel rotation
            ) 
            for ws, ss, dcm, sgn in zip(wheelSpeeds, self.state.steerSpeeds, wheel2body, self.whSgn)
        ])
